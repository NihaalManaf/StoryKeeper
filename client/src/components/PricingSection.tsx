...
const pricingPlans = [
  ...
  },
  {
    ...
    features: [
      ...
      { name: "Luxury hardcover with foil", included: true },
      { name: "Advanced photo personalization", included: true },
      { name: "Digital + audiobook version", included: true },
    ],
    popular: false,
    ...
    <section id="pricing" className="py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold font-['Quicksand'] mb-4">Simple, Transparent Pricing</h2>
          <p className="text-xl max-w-2xl mx-auto text-gray-600">Choose the perfect package for your family's story</p>
        </div>
          ...
          {pricingPlans.map((plan, index) => (
            <div 
              ...
              className={`bg-white rounded-[12px] shadow-lg overflow-hidden transition duration-500 hover:shadow-xl ${
                plan.popular ? 'md:-translate-y-4 scale-105 shadow-2xl relative' : ''
              }`}
            >
              {plan.popular && (
                <div className="absolute top-0 right-0 bg-[#FF6B6B] text-white px-4 py-1 font-bold">Most Popular</div>
              )}
              <div className="p-8 border-b">
                <h3 className="text-2xl font-bold font-['Quicksand'] mb-2">{plan.name}</h3>
        ...
        <div className="mt-12 text-center">
          <p className="text-gray-600 mb-4">Need multiple copies or school orders?</p>
          <a href="#contact" className="text-[#4ECDC4] font-bold hover:underline">Contact us for special bulk pricing</a>
        </div>
      </div>
    </section>
  );
